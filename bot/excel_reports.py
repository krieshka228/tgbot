...
def build_monthly_report(orders: list, users: list) -> bytes:
    ...
    ws1.title = "Заказы"
    headers1 = [
        "№ заказа", "Клиент", "Телефон", "Способ доставки", "Адрес",
        "Товар", "Кол-во", "Цена, ₽", "Сумма, ₽", "Дата заказа",
    ]
    ...
    for order in orders:
        ...
        if not order.items:
            ...
            ws1.cell(row=row, column=2, value=user_name).border = _thin_border()
            ws1.cell(row=row, column=3, value=phone).border = _thin_border()
            ws1.cell(row=row, column=4, value=method).border = _thin_border()
            ws1.cell(row=row, column=5, value=address).border = _thin_border()
            ws1.cell(row=row, column=6, value="(нет позиций)").border = _thin_border()
            ws1.cell(row=row, column=7, value=0).border = _thin_border()
            ws1.cell(row=row, column=8, value=0).border = _thin_border()
            ws1.cell(row=row, column=9, value=0).border = _thin_border()
            ws1.cell(row=row, column=10, value=date_str).border = _thin_border()
            row += 1
        else:
            for item in order.items:
                ...
                ws1.cell(row=row, column=4, value=method).border = _thin_border()
                ws1.cell(row=row, column=5, value=address).border = _thin_border()
                ws1.cell(row=row, column=6, value=name).border = _thin_border()
                ws1.cell(row=row, column=7, value=item.quantity).border = _thin_border()
                ws1.cell(row=row, column=8, value=round(item.price_at_order, 2)).border = _thin_border()
                ws1.cell(row=row, column=9, value=round(subtotal, 2)).border = _thin_border()
                ws1.cell(row=row, column=10, value=date_str).border = _thin_border()
                row += 1
